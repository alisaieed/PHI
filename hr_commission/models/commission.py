...
class Commission(models.Model):
    ...
    generate_rest_commission = fields.Boolean('Generate Rest Commission')
    ...
    def _compute_source_move_id(self):	
        for commission in self:
            if commission.source_move_id:
                ...
                commission.base_total = commission.source_move_id.amount_total_signed - commission.source_move_id.amount_residual
                commission.base_amount = commission.source_move_id.amount_for_commission(commission)

    @api.onchange('contract_id', 'rate_type', 'rate')
    ...
    def action_generate_rest_commission(self):
        amount = self.source_move_id.amount_total_signed - self.source_move_id.amount_residual - self.base_total
        self.create({'employee_id': self.employee_id.id,
    ...
    def action_confirm(self):
        ...
        if self.generate_rest_commission:
            self.action_generate_rest_commission()
