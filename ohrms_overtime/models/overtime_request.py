...
class HrOverTime(models.Model):
    ...
    current_user = fields.Many2one('res.users', string="Current User", related='employee_id.user_id', default=lambda self: self.env.uid, store=True)
    ...
    def _get_hour_amount(self):
        if self.overtime_type_id.rule_line_ids and self.duration_type == 'hours':
            for recd in self.overtime_type_id.rule_line_ids:
                if recd.from_hrs < self.days_no_tmp <= recd.to_hrs and self.sudo().contract_id:
                    if self.sudo().contract_id.over_hour:
                        cash_amount = self.sudo().contract_id.over_hour * recd.hrs_amount
    ...
    def _onchange_date(self):
        holiday = False
        if self.sudo().contract_id and self.date_from and self.date_to:
            for leaves in self.sudo().contract_id.resource_calendar_id.global_leave_ids:
